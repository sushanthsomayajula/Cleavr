...
1. Extracts the pure compute core straight out of `website/qflow.html` (the block between the `QFLOW-CORE-START` / `QFLOW-CORE-END` markers) and loads it in Node. There is no second copy of the math, and the test runs the exact code the page ships, so the two can't drift.
