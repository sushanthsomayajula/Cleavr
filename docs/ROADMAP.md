...
7. ~~Build absolute receptor quantification (not just relative signal).~~ Done: `code/receptor_quantification.py` reads `flow_cytometry/calibration_beads.csv` (bead populations with certified antibody-binding capacity), fits a calibration curve per experiment date (tries both linear and log-log least-squares regression, keeps whichever has the higher R^2, implemented by hand for auditability), and converts each sample's MFI into a background-subtracted receptors-per-cell estimate. Tested end-to-end with synthetic data; no real calibration runs yet.
8. Next: run the first real flow cytometry experiment, with calibration beads included, and see whether FTO's protein-level pattern (now with an actual receptor count, not just a fold-change) matches its RNA-level pattern.
